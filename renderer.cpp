...
ID3D11Buffer*			Renderer::m_InstancingBuffer = NULL;
...
D3DXVECTOR4 Renderer::clearColor = { 184 / 255.0f, 210 / 255.0f, 1.0f, 1.0f };
...
void Renderer::Init()
{
	...
	//blendDesc.AlphaToCoverageEnable = TRUE;//半透明オブジェクトは正しく描画できない
	...
	bufferDesc.ByteWidth = sizeof(D3DXVECTOR4);//作成するバッファのサイズを設定
	//バッファを作成してそのインターフェイスを受け取る
	m_Device->CreateBuffer(&bufferDesc, NULL, &m_ParameterBuffer);
	//データを受け取りたいシェーダーへインターフェイスをセット
	m_DeviceContext->VSSetConstantBuffers(6, 1, &m_ParameterBuffer);
	m_DeviceContext->PSSetConstantBuffers(6, 1, &m_ParameterBuffer);

	//インスタンシング用
	bufferDesc.ByteWidth = sizeof(InstancingData);//作成するバッファのサイズを設定
//バッファを作成してそのインターフェイスを受け取る
	m_Device->CreateBuffer(&bufferDesc, NULL, &m_InstancingBuffer);
	//データを受け取りたいシェーダーへインターフェイスをセット
	m_DeviceContext->VSSetConstantBuffers(9, 1, &m_InstancingBuffer);
	m_DeviceContext->PSSetConstantBuffers(9, 1, &m_InstancingBuffer);
...
//バッファを作成してそのインターフェイスを受け取る
	...
	io.Fonts->AddFontFromFileTTF("C:\Windows\Fonts\ITCEDSCR.TTF",10.0f,nullptr,io.Fonts->GetGlyphRangesJapanese());
...
void Renderer::Begin()
{
	...
	//float clearColor[4] = { 184/255.0f, 210 / 255.0f, 1.0f, 1.0f };
...
void Renderer::SetInstancingData(InstancingData s_data) {

	m_DeviceContext->UpdateSubresource(m_InstancingBuffer, 0, NULL, &s_data, 0, 0);

}

void Renderer::SetCameraPosition(D3DXVECTOR4 CameraPosition)
{
...
void Renderer::SetCulling(D3D11_CULL_MODE s_mode) {

	// ラスタライザステート設定
	D3D11_RASTERIZER_DESC rasterizerDesc{};
	rasterizerDesc.FillMode = D3D11_FILL_SOLID;
	rasterizerDesc.CullMode = s_mode;
	rasterizerDesc.DepthClipEnable = TRUE;
	rasterizerDesc.MultisampleEnable = FALSE;
